...
static const char base64_chars1[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
void Base64::base64_decode(unsigned char const* encoded_string, unsigned int in_len, unsigned char* decode_string, unsigned int* out_len)
{
	int i = 0;
	int j = 0;
	int in_ = 0;
	...
	while (in_len-- && ( encoded_string[in_] != '=') && is_base64(encoded_string[in_])) {
		...
		if (i ==4) {
			...
			for (i = 0; (i < 3); i++)
				memcpy(&decode_string[off++], &char_array_3[i], 1);
	...
	if (i) {
		...
		for (j = 0; (j < i - 1); j++) memcpy(&decode_string[off++], &char_array_3[j], 1);
	}
...
std::string Base64::base64_encode(unsigned char const* bytes_to_encode, unsigned int in_len)
{
	...
	while (in_len--) {
		...
		if (i == 3) {
			...
			for(i = 0; (i <4) ; i++)
			{
				char ch1 = base64_chars1[char_array_4[i]];
				ret += ch1;
			}
			i = 0;
		}
	...
	if (i)
	{
		...
		for (j = 0; (j < i + 1); j++)
		{
			char ch2 = base64_chars1[char_array_4[j]];
			ret += ch2;
		}
